...
def autopilot2_update():
    ...
    global player_speed_change, player_position, heading_degree, player_position_change
...
def autopilot3_turn_update():
    ...
    error1 = (target_heading2 - heading_degree + 540) % 360 - 180
    if abs(error1) > 0.25:
        heading_degree += error1 / 30 #it changes the heading by the error / 30, which gives an impression that it turns sharply first, and then smoother and smoother.
    else:
        heading_degree = target_heading2
...
def score1(velocity, distance, angle):
    ...
    return 1 / (distance * (1 + angle_penalty))
...
def autopilot4_update():
    ...
    for j in range(points_number):
        ...
        current_score = score1(player_speed, distance_to, error_angle)
...
def handle_input():
    global heading_degree, player_speed
...
def is_collection():
    ...
    for j in range(points_number):
        ...
        distance = player_position.distance_to(point_position)

        #if the distance is small enough it looks it is hit, give us the index (j) of the point, which is going to be used in the collision_happened() function
        if distance <= 24:
            return j
    return None
